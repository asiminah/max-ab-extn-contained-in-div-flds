...
procedure VerifyThm13Index1()
	...
	print "==================";
	for n in [3,9,27] do
		N := GL2CartanNormalizer(-3,n);
		CC := CommutatorSubgroup(N);
		print "The size of the commutator subgroup mod", n, "is", #CC;
	end for;
	print "==================";
	//Base case:
...
procedure VerifyThm13Index2()
	...
	for n in [3,9,27] do
		MatList := [];
		for b in [1..n] do
			MatList := Append(MatList, [1,b,delta*b,1]);
		end for;
		N1 := sub<GL(2,Integers(n))|MatList,[-1,0,0,1]>;
		N2 := sub<GL(2,Integers(n))|MatList,[1,0,0,-1]>;
		CC1 := CommutatorSubgroup(N1); 
		CC2 := CommutatorSubgroup(N2);
		if #CC1 eq #CC2 then
			print "The size of the commutator subgroup mod", n, "is", #CC1;
		else
			print "Problem";
		end if;
	end for;
...
procedure VerifyThm13Index3and6()
	...
	print "Image G^1";
	print "==================";
	print "Size of commutator subgroup for index 3 images";
	print "==================";
	delta := -3/4;
	for n in [3,9,27] do
		MatList := [];
		for a,b in [1..n] do
			if (a mod 3 ne 0) and (b mod 3 eq 0) then 
				MatList := Append(MatList, [a,b,delta*b,a]);
			end if;
		end for;
		G1a1 := sub<GL(2,Integers(n))|MatList,[-1,0,0,1]>;
		G1a2 := sub<GL(2,Integers(n))|MatList,[1,0,0,-1]>;
		CC1 := CommutatorSubgroup(G1a1); 
		CC2 := CommutatorSubgroup(G1a2);
		if #CC1 eq #CC2 then
			print "The size of the commutator subgroup mod", n, "is", #CC1;
		else
			print "Problem";
	...
	print "==================";
	print "Size of commutator subgroup for index 6 images";
	print "==================";
	delta := -3/4;
	for n in [3,9,27] do
		MatList := [];
		for a,b in [1..n] do
			if (a mod 3 eq 1) and (b mod 3 eq 0) then 
				MatList := Append(MatList, [1,b,delta*b,1]);
			end if;
		end for;
		G1a1 := sub<GL(2,Integers(n))|MatList,[-1,0,0,1]>;
	...
	print "==================";
	for n in [3,9,27] do
		G2a1 := sub<GL(2,Integers(n))|[2,0,0,2],[1,1,-3/4,1],[-1,0,0,1]>;
		G2a2 := sub<GL(2,Integers(n))|[2,0,0,2],[1,1,-3/4,1],[1,0,0,-1]>;
		CC1 := CommutatorSubgroup(G2a1);
		CC2 := CommutatorSubgroup(G2a2);
		if #CC1 eq #CC2 then
			print "The size of the commutator subgroup mod", n, "is", #CC1;
		else
			print "Problem";
	...
	print "==================";
	for n in [3,9,27] do
		G2a1 := sub<GL(2,Integers(n))|[4,0,0,4],[1,1,-3/4,1],[-1,0,0,1]>;
		G2a2 := sub<GL(2,Integers(n))|[4,0,0,4],[1,1,-3/4,1],[1,0,0,-1]>;
		CC1 := CommutatorSubgroup(G2a1);
		CC2 := CommutatorSubgroup(G2a2);
		if #CC1 eq #CC2 then
			print "The size of the commutator subgroup mod", n, "is", #CC1;
	...
	print "==================";
	for n in [3,9,27] do
		G3a1 := sub<GL(2,Integers(n))|[2,0,0,2],[-5/4,1/2,-3/8,-5/4],[-1,0,0,1]>;
		G3a2 := sub<GL(2,Integers(n))|[2,0,0,2],[-5/4,1/2,-3/8,-5/4],[1,0,0,-1]>;
		CC1 := CommutatorSubgroup(G3a1);
		CC2 := CommutatorSubgroup(G3a2);
		if #CC1 eq #CC2 then
			print "The size of the commutator subgroup mod", n, "is", #CC1;
		else
			print "Problem";
	...
	print "==================";
	for n in [3,9,27] do
		G3a1 := sub<GL(2,Integers(n))|[4,0,0,4],[-5/4,1/2,-3/8,-5/4],[-1,0,0,1]>;
		G3a2 := sub<GL(2,Integers(n))|[4,0,0,4],[-5/4,1/2,-3/8,-5/4],[1,0,0,-1]>;
		CC1 := CommutatorSubgroup(G3a1);
		CC2 := CommutatorSubgroup(G3a2);
		if #CC1 eq #CC2 then
			print "The size of the commutator subgroup mod", n, "is", #CC1;
...
procedure VerifyProp54Index3()
	...
	print "==================";
	for n in [2,4,8,16] do
	G1 := sub<GL(2,Integers(n))|[-1,0,0,-1],[7,4,-4,3],[3,6,-6,-3],[0,1,1,0]>;
	G2 := sub<GL(2,Integers(n))|[-1,0,0,-1],[7,4,-4,3],[3,6,-6,-3],[0,-1,-1,0]>;
	CC1 := CommutatorSubgroup(G1);
	CC2 := CommutatorSubgroup(G2);
	if #CC1 eq #CC2 then
		print "The size of the commutator subgroup mod", n, "is", #CC1;
	else
		print "Problem";
...
procedure VerifyThm14bi()
	...
	print "==================";
	for n in [2,4,8,16] do
		N1 := GL2CartanNormalizer(-12,n);
		N2 := GL2CartanNormalizer(-28,n);
		CC1 := CommutatorSubgroup(N1);
		CC2 := CommutatorSubgroup(N2);
		if #CC1 eq #CC2 then
			print "The size of the commutator subgroup mod", n, "is", #CC1;
...
procedure VerifyThm14bii()
	...
	G:=GL(2,F);
	print "==================";
	print "Size of commutator subgroup for index 1 images";
	print "==================";
	for n in [2,4,8,16] do
		N := GL2CartanNormalizer(-4,n);
		CC := CommutatorSubgroup(N);
		print "The size of the commutator subgroup mod", n, "is", #CC;
	end for;
	print "==================";
	print "Size of commutator subgroup for index 2 images G_{2,a}";
	print "==================";
	for n in [2,4,8,16] do
	G11 := sub<GL(2,Integers(n))|[-1,0,0,1],[-1,0,0,-1],[3,0,0,3],[1,2,-2,1]>;
	G12 := sub<GL(2,Integers(n))|[1,0,0,-1],[-1,0,0,-1],[3,0,0,3],[1,2,-2,1]>;
	G13 := sub<GL(2,Integers(n))|[0,1,1,0],[-1,0,0,-1],[3,0,0,3],[1,2,-2,1]>;
	G14 := sub<GL(2,Integers(n))|[0,-1,-1,0],[-1,0,0,-1],[3,0,0,3],[1,2,-2,1]>;
	CC11 := CommutatorSubgroup(G11);
	CC12 := CommutatorSubgroup(G12);
	CC13 := CommutatorSubgroup(G13);
	CC14 := CommutatorSubgroup(G14);
	if (#CC11 eq #CC12) and (#CC12 eq #CC13) and (#CC13 eq #CC14) then
		print "The size of the commutator subgroup mod", n, "is", #CC11;
	else
		print "Problem";
	end if;
	end for;
	print "==================";
	print "Size of commutator subgroup for index 2 images G_{2,b}";
	print "==================";
	for n in [2,4,8,16] do
	G21 := sub<GL(2,Integers(n))|[-1,0,0,1],[-1,0,0,-1],[3,0,0,3],[2,1,-1,2]>;
	G22 := sub<GL(2,Integers(n))|[1,0,0,-1],[-1,0,0,-1],[3,0,0,3],[2,1,-1,2]>;
	G23 := sub<GL(2,Integers(n))|[0,1,1,0],[-1,0,0,-1],[3,0,0,3],[2,1,-1,2]>;
	G24 := sub<GL(2,Integers(n))|[0,-1,-1,0],[-1,0,0,-1],[3,0,0,3],[2,1,-1,2]>;
	CC21 := CommutatorSubgroup(G21);
	CC22 := CommutatorSubgroup(G22);
	CC23 := CommutatorSubgroup(G23);
	CC24 := CommutatorSubgroup(G24);
	if (#CC21 eq #CC22) and (#CC22 eq #CC23) and (#CC23 eq #CC24) then
		print "The size of the commutator subgroup mod", n, "is", #CC21;
	else
		print "Problem";
	end if;
	end for;
	print "==================";
	print "Size of commutator subgroup for index 4 images G_{4,a} and G_{4,b}";
	print "==================";
	for n in [2,4,8,16] do
		G11 := sub<GL(2,Integers(n))|[-1,0,0,1],[5,0,0,5],[1,2,-2,1]>;
		G12 := sub<GL(2,Integers(n))|[1,0,0,-1],[5,0,0,5],[1,2,-2,1]>;
		G13 := sub<GL(2,Integers(n))|[0,1,1,0],[5,0,0,5],[1,2,-2,1]>;
		G14 := sub<GL(2,Integers(n))|[0,-1,-1,0],[5,0,0,5],[1,2,-2,1]>;
		G21 := sub<GL(2,Integers(n))|[-1,0,0,1],[5,0,0,5],[-1,-2,2,-1]>;
		G22 := sub<GL(2,Integers(n))|[1,0,0,-1],[5,0,0,5],[-1,-2,2,-1]>;
		G23 := sub<GL(2,Integers(n))|[0,1,1,0],[5,0,0,5],[-1,-2,2,-1]>;
		G24 := sub<GL(2,Integers(n))|[0,-1,-1,0],[5,0,0,5],[-1,-2,2,-1]>;
		CC11 := CommutatorSubgroup(G11);
		CC12 := CommutatorSubgroup(G12);
		CC13 := CommutatorSubgroup(G13);
		CC14 := CommutatorSubgroup(G14);
		...
		CC23 := CommutatorSubgroup(G23);
		CC24 := CommutatorSubgroup(G24);
		if (#CC11 eq #CC12) and (#CC12 eq #CC13) and (#CC13 eq #CC14) and (#CC14 eq #CC21) and (#CC21 eq #CC22) and (#CC22 eq #CC23) and (#CC23 eq #CC24) then
			print "The size of the commutator subgroup mod", n, "is", #CC11;
		else
			print "Problem";
		end if;
	...
	print "==================";
	print "Size of commutator subgroup for index 4 images G_{4,c} and G_{4,d}";
	print "==================";
	for n in [2,4,8,16] do
		G11 := sub<GL(2,Integers(n))|[-1,0,0,1],[-3,0,0,-3],[2,-1,1,2]>;
		G12 := sub<GL(2,Integers(n))|[1,0,0,-1],[-3,0,0,-3],[2,-1,1,2]>;
		G13 := sub<GL(2,Integers(n))|[0,1,1,0],[-3,0,0,-3],[2,-1,1,2]>;
		G14 := sub<GL(2,Integers(n))|[0,-1,-1,0],[-3,0,0,-3],[2,-1,1,2]>;
		G21 := sub<GL(2,Integers(n))|[-1,0,0,1],[-3,0,0,-3],[-2,1,-1,-2]>;
		G22 := sub<GL(2,Integers(n))|[1,0,0,-1],[-3,0,0,-3],[-2,1,-1,-2]>;
		G23 := sub<GL(2,Integers(n))|[0,1,1,0],[-3,0,0,-3],[-2,1,-1,-2]>;
		G24 := sub<GL(2,Integers(n))|[0,-1,-1,0],[-3,0,0,-3],[-2,1,-1,-2]>;
		CC11 := CommutatorSubgroup(G11);
		CC12 := CommutatorSubgroup(G12);
		CC13 := CommutatorSubgroup(G13);
		CC14 := CommutatorSubgroup(G14);
...
procedure VerifyThm14biiiIndex1()
	...
	print "==================";
	for n in [2,4,8,16] do
		N1 := GL2CartanNormalizer(-8,n);
		N2 := GL2CartanNormalizer(-16,n);
		CC1 := CommutatorSubgroup(N1);
		CC2 := CommutatorSubgroup(N2);
		if #CC1 eq #CC2 then
			print "The size of the commutator subgroup mod", n, "is", #CC1;
...
procedure VerifyThm14biiiIndex2()
	...
	print "==================";
	for n in [2,4,8,16] do
		G11 := sub<GL(2,Integers(n))|[-1,0,0,1],[3,0,0,3],[1,1,-2,1]>;
		G12 := sub<GL(2,Integers(n))|[1,0,0,-1],[3,0,0,3],[1,1,-2,1]>;
		G13 := sub<GL(2,Integers(n))|[-1,0,0,1],[5,0,0,5],[1,1,-2,1]>;
		G14 := sub<GL(2,Integers(n))|[1,0,0,-1],[5,0,0,5],[1,1,-2,1]>;
		G21 := sub<GL(2,Integers(n))|[-1,0,0,1],[3,0,0,3],[-1,-1,2,-1]>;
		G22 := sub<GL(2,Integers(n))|[1,0,0,-1],[3,0,0,3],[-1,-1,2,-1]>;
		G23 := sub<GL(2,Integers(n))|[-1,0,0,1],[5,0,0,5],[-1,-1,2,-1]>;
		G24 := sub<GL(2,Integers(n))|[1,0,0,-1],[5,0,0,5],[-1,-1,2,-1]>;
		CC11 := CommutatorSubgroup(G11);
		CC12 := CommutatorSubgroup(G12);
		CC13 := CommutatorSubgroup(G13);
		CC14 := CommutatorSubgroup(G14);
	...
	print "==================";
	for n in [2,4,8,16] do
		G11 := sub<GL(2,Integers(n))|[-1,0,0,1],[3,0,0,3],[1,1,-4,1]>;
		G12 := sub<GL(2,Integers(n))|[1,0,0,-1],[3,0,0,3],[1,1,-4,1]>;
		G13 := sub<GL(2,Integers(n))|[-1,0,0,1],[5,0,0,5],[1,1,-4,1]>;
		G14 := sub<GL(2,Integers(n))|[1,0,0,-1],[5,0,0,5],[1,1,-4,1]>;
		G21 := sub<GL(2,Integers(n))|[-1,0,0,1],[3,0,0,3],[-1,-1,4,-1]>;
		G22 := sub<GL(2,Integers(n))|[1,0,0,-1],[3,0,0,3],[-1,-1,4,-1]>;
		G23 := sub<GL(2,Integers(n))|[-1,0,0,1],[5,0,0,5],[-1,-1,4,-1]>;
		G24 := sub<GL(2,Integers(n))|[1,0,0,-1],[5,0,0,5],[-1,-1,4,-1]>;
		CC11 := CommutatorSubgroup(G11);
		CC12 := CommutatorSubgroup(G12);
		CC13 := CommutatorSubgroup(G13);
		CC14 := CommutatorSubgroup(G14);
